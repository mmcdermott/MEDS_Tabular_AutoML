...
def test_integration(tmp_path):
    MEDS_cohort_dir = Path(tmp_path) / "processed"
    ...
    stderr, stdout_ws = run_command(
        "generate-optuna-intervals",
        ["tabularization.window_inclusion", "30d,365d,full"],
        {},
        "generate-optuna-intervals window sizes",
    )
    stderr, stdout_agg = run_command(
        "generate-optuna-intervals",
        ["tabularization.agg_inclusion", "code/count,value/sum"],
        {},
        "generate-optuna-intervals aggs",
    )
    ...
    stderr, stdout = run_command("meds-tab-cache-task", [], cache_config, "task_specific_caching")
    ...
    stderr, stdout = run_command("meds-tab-xgboost", [], xgboost_config, "xgboost_training")

    multi_run_xgboost_config = {
        "MEDS_cohort_dir": str(MEDS_cohort_dir.resolve()),
        "do_overwrite": False,
        "seed": 1,
        "hydra.verbose": True,
    ...
    ws_command = "$(generate-optuna-intervals tabularization.window_inclusion 30d,365d,full)"
    agg_command = "$(generate-optuna-intervals tabularization.agg_inclusion code/count,value/sum)"
    stderr, stdout = run_command(
        "meds-tab-xgboost",
        ["--multirun", ws_command, agg_command],
        multi_run_xgboost_config,
        "multi_run_xgboost_training",
    )
